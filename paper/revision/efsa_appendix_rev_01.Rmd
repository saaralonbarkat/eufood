...
load("G:/My Drive/University/R/EFSA/.RData")
source("G:/My Drive/University/R/EFSA/code/efsa_dm_01.R")
...
**One-factor model:**
```{r,echo = TRUE}
cfa.model.1factor <- ' policy.credibility  =~ credibility.pest.q1 + credibility.pest.q2 + credibility.pest.q3 + credibility.pest.q4 + credibility.pest.q5 + credibility.pest.q6 '

fit.1factor <- cfa(cfa.model.1factor, 
           std.lv=TRUE,  
           missing="fiml",  
           data=efsa_00)
...
```{r,echo = TRUE}
cfa.model.3factors <- ' non.interference  =~ credibility.pest.q1 + credibility.pest.q2 + credibility.pest.q3
              expert.based =~ credibility.pest.q4 + credibility.pest.q5
              stability   =~ credibility.pest.q6 '

fit.3factors <- cfa(cfa.model.3factors, 
           std.lv=TRUE,  
           missing="fiml",  
           data=efsa_00)
...
```{r}
coding.representation <- read_csv("G:/My Drive/University/R/EFSA/data/repersentation.csv")
```
...
tmod.expertise.21 <- update(tmod.considerations.21, credibility.pest.expertise ~ . )


tmod.commitment.21 <- update(tmod.considerations.21, credibility.pest.commitment ~ . )

```

```{r, results="asis",echo=FALSE,warning=FALSE,message=FALSE}

stargazer(tmod.commitment.21,
          tmod.expertise.21,
