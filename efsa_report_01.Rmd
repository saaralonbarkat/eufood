...
efsa_00 %>% 
  ...
  geom_bar(stat="identity",width = 0.75,fill="dodgerblue2",alpha=0.8)+
  scale_y_continuous(limits = c(0,nrow(t1)*0.40),breaks = seq(0,200,10))+
  scale_x_discrete(name = "")+
  coord_flip()+
  theme_tufte()
```
...
<br>
```{r, fig.width=6}
ggplot(efsa_00,aes(organization.type.raw,prior.beliefs.food))+
  geom_boxplot(width = 0.5,
               outlier.colour = "white")+  
...
<br>
```{r, fig.width=6}
ggplot(efsa_00,aes(organization.type.raw,supports.pesticides))+
  geom_boxplot(width = 0.5,
               outlier.colour = "white")+  
...
```{r}
efsa_00 %>% 
  select(idependence.polit.efsa:idependence.polit.echa) %>%   gather() %>% 
...
ggplot(efsa_00,aes(organization.type.raw,idependence.polit.efsa))+
  ...
  geom_jitter(width = 0.2, height = 0.2,size=1,colour="dodgerblue2",alpha=0.6)+
  xlab("Organization type")+
  scale_y_continuous(name="Independence from politicians (5=high)",limits = c(-6,6),breaks = -5:5)+
  coord_flip()+
  theme_tufte()
...
ggplot(efsa_00,aes(efsa_00$idependence.polit.efsa,
  ...
  geom_jitter(width = 0.1,height = 0.1,size=2,colour="dodgerblue2",alpha=0.6)+
