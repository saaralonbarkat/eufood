...
n.reach_cases = efsa_00 %>% filter(!(condition %in% NA))%>% nrow()
n.reach_independence = efsa_00 %>% filter(!(idependence.polit.efsa %in% NA))%>% nrow()
...
efsa_00 <- efsa_00 %>% 
  ...
  sjp.frq(efsa_00$timer[efsa_00$timer<60],type="histogram",
        show.mean = T,
        geom.size = 2,
...
First, to test randomo assignment by Qualtrics software, I generated automatic random test responses of n=2000.  
```{r}
...
```{r}
model.treatment.t1 <- glm(treatment~female+
         age+
         trust.eu.commission+
         trust.eu.parliament+
         trust.eu.council+
...
dwplot(list(Treatment=model.treatment.t1,Risk=model.risk.t1),
...
```{r}
efsa_00 %>% 
  filter(!(idependence.polit.efsa %in% NA)) %>%
  mutate (treatment.lab = treatment %>% Recode("0='Control';1='Treatment'")) %>% 
...
```{r}
efsa_00 %>% 
  filter(!(idependence.industry.efsa %in% NA)) %>%
  mutate (treatment.lab = treatment %>% Recode("0='Control';1='Treatment'")) %>% 
...
efsa_00 %>% 
  filter(!(high.risk %in% NA)) %>%
  mutate(high.risk.lab = high.risk %>% Recode("0='Low risk'; 1='High risk'")) %>%
ggplot(aes(credibility.pest.index,
         credibility.efsa.pre))+
  geom_jitter(width = 0.05,height = 0.05,size=2,colour="dodgerblue2",alpha=0.5)+
  geom_smooth(method="lm",colour="black")+
...
efsa_00 %>% 
  filter(!(high.risk %in% NA)) %>%
  mutate(high.risk.lab = high.risk %>% Recode("0='Low risk'; 1='High risk'")) %>%
ggplot(aes(prior.beliefs.food,
           credibility.pest.index))+
  geom_jitter(width = 0.05,height = 0.05,size=2,colour="dodgerblue2",alpha=0.5)+
  geom_smooth(method="lm",colour="black")+
  scale_x_continuous(name="Brior beliefs food index",breaks=-5:5)+
  scale_y_continuous(name = "Decision creibility",breaks=-5:5)+
  ggtitle("")+
  facet_wrap(~high.risk.lab)+
  theme_tufte()
...
```{r}
efsa_00 %>% 
  filter(!(credibility.pest.index %in% NA)) %>%
  mutate (treatment.lab = treatment %>% Recode("0='Control';1='Treatment'")) %>% 
ggplot(aes(credibility.pest.index,fill=factor(treatment.lab)))+ 
