...
## **Differences between organization groups (industry, NGOs, others):** 

* All three groups percieve EFSA as a relatively credible regulator. NGO participants perceive EFSA as slightly less credible (pre-manipulation), compared with participants form industry and others. **(Fig. 1)**

* NGO participants tend to perceive pesticides as a risk and tend to perceive food companies as greedy. Industry participants tend to view pesticides and food companies favorably. Others are neutral. **(Fig. 1)**  

* No significant differences between the group regarding perceived independence from political stakeholders. Regarding independence from industry, NGO participants tend to think that EFSA relatively dependent, whereas industry members tend to think that it is independent. Others are neutral. **(Fig. 2)**   
...
ggplot(aes(x=key,y=value,color=organization.type.short,shape=organization.type.short))+
  ...
  coord_flip()+
  theme_tufte()+
  labs(title = "Fig. 1: Prior beliefs across agency type",
              subtitle = "",
...
ggplot(aes(x=key,y=value,color=organization.type.short,shape=organization.type.short))+
  ...
  coord_flip()+
  theme_tufte()+
  labs(title = "Fig. 2: Perceived EFSA independence across agency type",
              subtitle = "",
              caption = "Note: Red dots and bars represent means and 90% CIs")+
  theme(
  plot.caption = element_text(hjust = 0)
...
* EFSA's decision to ban the Amitrole pesticide perceived as more credible from their decision to approve the Flup. pesticide. Both decisions are relatively credible, with the exception that the approval decision is not perceived as a credible commitment. **(Fig. 3)**

* Decision credibility index has a reasonable reliability (Cronbach's alpha = ~0.85). Below we only refer to this index. **(Table 1)**

* The approve pesticide decision is perceived as relatively credible by industry participants, and relatively uncredible by NGO participants. Other participants are slightly positive (but less than industry). The approve pesticide decision is perceived as relatively credible by all three groups with no majoe differences between them. **(Fig. 4)**

* Beyond the differences between organization types, the credibility of the two decisions is positively associated with pre-manipulation credibility of EFSA and perceived independence of EFSA from industry. The ban decision is also positively associated with perceived independence from political stakeholders. Credibility of ban decision is also associated with greater prior support for pesticides. Credibility of approve decision is negatively correlated with support for pesticides and perceived greediness of the food industry, but these coefficients are not sufficiently statistically significant. **(Fig. 5)**
...
ggplot(aes(x=key,y=value,color=ban.pesticide.lab,shape=ban.pesticide.lab))+
  ...
  ylab("")+
  theme_tufte()+
    labs(title = "Fig. 3: Decision credibility by case",
              subtitle = "",
              caption = "Note: Red dots and bars represent means and 90% CIs")+
  theme(
  plot.caption = element_text(hjust = 0)
...
Table 1: Item analysis for credibility index
```{r}
t1 <- efsa_00 %>% 
  select(credibility.pest.q1:credibility.pest.q6) %>% 
  mutate(credibility.pest.q2 = -1*credibility.pest.q2,
         ...
         credibility.pest.q6 = -1*credibility.pest.q6) %>%
  select(credibility.pest.q1:credibility.pest.q6) %>% 
  sjt.itemanalysis(remove.spaces	=T,
                   show.corr.matrix	=T)
```
...
ggplot(aes(x=organization.type.short,y=credibility.pest.index,
  ...
  # facet_wrap(~ban.pesticide.lab)+
  theme_tufte() +
  labs(title = "Fig. 4: Decision credibility across organization groups",
              subtitle = "",
              caption = "Note: Red dots and bars represent means and 90% CIs")+
  theme(
  plot.caption = element_text(hjust = 0)
...
dwplot(list(Approve.pesticide=tmod1,Ban.pesticide=tmod2
  ...
  theme_tufte()+
  labs(title = "Fig. 5: Determinants of decision credibility",
              subtitle = "",
...
* Independence treatment significanly increasese perceived independence of EFSA from politicians (*p* = `r t.test(independence.polit.efsa~treatment,efsa_00)[["p.value"]] %>% round(2)`, cohen's d = `r cohen.d(efsa_00$independence.polit.efsa~efsa_00$treatment)[["estimate"]] %>% abs() %>%  round(2)`), as well as percieved independence from industry (*p* = `r t.test(independence.industry.efsa~treatment,efsa_00)[["p.value"]] %>% round(2)`, cohen's d = `r cohen.d(efsa_00$independence.industry.efsa~efsa_00$treatment)[["estimate"]] %>% abs() %>%  round(2)`). The effect is similar for both decisions. **(Fig. 6)**

* Independence treatment increasese credibility of approve decision (*p* = `r t.test(credibility.pest.index~treatment,efsa_00_approve.pest)[["p.value"]] %>% round(2)`, cohen's d = `r cohen.d(efsa_00_approve.pest$credibility.pest.index~efsa_00_approve.pest$treatment)[["estimate"]] %>% abs() %>%  round(2)`), but not of ban decision. **(Fig. 7)** 

* The treatement effect on credibility is mediated by perceived independence from politicians and from industry. 
...
ggplot(aes(x=treatment.lab,y=value))+
  geom_boxplot(width = 0.5)+
      stat_summary(fun.data = mean_cl_normal, geom = "errorbar", fun.args=list(conf.int=0.90), colour = "red",alpha=0.5,width=0.1,size=1,position=position_dodge(0.5)) + 
  stat_summary(fun.y = mean, geom = "point", colour = "red",alpha=0.5,size=2,position=position_dodge(0.5))+
...
scale_y_continuous(limits = c(-5,5),breaks=-5:5)+
  ...
  facet_wrap(~key)+
theme_tufte()+
    labs(title = "Fig. 6: Treatment effect on perceived independence",
              subtitle = "",
              caption = "Note: Red dots and bars represent means and 90% CIs")+
  theme(
  plot.caption = element_text(hjust = 0)
...
ggplot(aes(x=treatment.lab,y=credibility.pest.index))+
  ...
  facet_wrap(~ban.pesticide.lab)+
  theme_tufte()+
      labs(title = "Fig. 7: Treatment effect on decision credibility",
              subtitle = "",
              caption = "Note: Red dots and bars represent means and 90% CIs")+
  theme(
  plot.caption = element_text(hjust = 0)
...
<br>
```{r}
tmod1 <- lm(credibility.pest.index ~ treatment,efsa_00_approve.pest)
tmod2 <- lm(credibility.pest.index ~ treatment+independence.polit.efsa,efsa_00_approve.pest)
tmod3 <- lm(credibility.pest.index ~ treatment+independence.polit.efsa+independence.industry.efsa,efsa_00_approve.pest)
tmod4 <- lm(credibility.pest.index ~ treatment,efsa_00_ban.pest)
tmod5 <- lm(credibility.pest.index ~ treatment+independence.polit.efsa,efsa_00_ban.pest)
tmod6 <- lm(credibility.pest.index ~ treatment+independence.polit.efsa+independence.industry.efsa,efsa_00_ban.pest)
...
tab_model(tmod1,
          ...
          show.ci = F,             
          collapse.se = T,
          dv.labels = c(" ","Approve pesticide"," ", " " ,"Ban pesticide"," "),
          wrap.labels = 1,
          title = "Table 2: Treatment effect on credibility")
...
* The treatment effect on perceived independence is greater among the 'other' group. However, the moderation of organization type is marginally significant. **(Fig. 8)**      

* The treatment effect on credibility of decisions is not statistically different across the different groups. **(Fig. 9)**

* The treatment effect on credibility of decisions does not depend on their prior support for pesticides. **(Fig. 10)**
...
efsa_00 %>% 
  drop_na(credibility.pest.index) %>% 
ggplot(aes(x=treatment.lab,y=credibility.pest.index,color=treatment.lab))+
  geom_boxplot()+
  scale_y_continuous(name="Decision credibility INDEX",limits = c(-5,5),breaks=-5:5)+
  xlab("")+
  stat_summary(fun.data = mean_cl_normal, geom = "errorbar", fun.args=list(conf.int=0.90), colour = "red",alpha=0.5,width=0.1,size=1) + 
  ...
  theme_tufte()+
  labs(title = "Fig. 9:",
              subtitle = "Treatment effect on decision credibility by org. type",
              caption = "Note: Red dots and bars represent means and 90% CIs")+
  theme(
  plot.caption = element_text(hjust = 0)
  )
...
facet_wrap(~ban.pesticide.lab)+
  theme_tufte()+
  labs(title = "Fig. 9:",
              subtitle = "Treatment effect on credibility by org. type",
              caption = "Note: Red dots and bars represent means and 90% CIs")+
  theme(
  plot.caption = element_text(hjust = 0)
  )
...
**Fig. 10: Marginal treatment effect on prior support for pesticides**
...
inter.binning(Y = "credibility.pest.index", D = "treatment", X = "supports.pesticides", data = t1.approve, nbins = 3,na.rm = TRUE, main = "Ban pesticide")$graph +
  geom_hline(yintercept = 0, colour = "grey60", linetype = 2)+
  theme_tufte()  +
  labs(title = "Fig. 10b:",
              subtitle = "Approve pesticide",
              caption = "")+
  theme(
  plot.caption = element_text(hjust = 0)
  )
...
* Stakeholders perception of the credibility of a regulatory decision is shaped by their prior positions on the subject matter. Decisions that are congruent with their positions are perceived as more credible, than decisions incongruent with their positions. 

* We do not find support for the expectation that the effect of independence depends on congruence with prior positions. This entails that increasing independence may increase perceived credibility of agency's decisions, not only when decisions fit stakeholders' prior beliefs, but also when they contradict them. 
