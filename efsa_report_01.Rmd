...
efsa_00 <- efsa_00 %>% 
  mutate(organization.type.short = organization.type.raw %>% 
           Recode("'non-governmental nonprofit organization'='ngo';
                  c('trade association','company')='industry';
                  else='other'"))
...
```{r}
efsa_00 %>% 
  select(idependence.industry.efsa,
...
```{r}
efsa_00 %>% 
  select(starts_with("idependence."))%>% 
  sjt.corr(triangle = "lower",show.p=T,p.numeric=T)
```
<br>

```{r, fig.width=6}
ggplot(efsa_00,aes(organization.type.raw,idependence.polit.efsa))+
  geom_boxplot(width = 0.5,
               outlier.colour = "white")+  
  geom_jitter(width = 0.2, height = 0.2,size=1,colour="dodgerblue2",alpha=0.6)+
      stat_summary(fun.data = mean_cl_normal, geom = "errorbar", colour = "red",alpha=0.5,width=0.1,size=1) + 
...
<br>
```{r, fig.width=6}
ggplot(efsa_00,aes(organization.type.raw,idependence.industry.efsa))+
  geom_boxplot(width = 0.5,
               outlier.colour = "white")+  
  geom_jitter(width = 0.2, height = 0.2,size=1,colour="dodgerblue2",alpha=0.6)+
    stat_summary(fun.data = mean_cl_normal, geom = "errorbar", colour = "red",alpha=0.5,width=0.1,size=1) + 
...
drop_na(idependence.industry.efsa)%>%
  ggplot(aes(factor(idependence.industry.efsa),
           idependence.polit.efsa))+
    geom_boxplot(width = 0.5,
               outlier.colour = "white")+  
  geom_jitter(width = 0.2,height = 0.2,size=1,colour="dodgerblue2",alpha=0.5)+
...
library(effsize)
cohen.d(efsa_00$idependence.polit.efsa~efsa_00$treatment)
```
<br>
```{r}
efsa_00 %>% 
...
lm(idependence.polit.efsa~treatment*ban.pesticide,efsa_00) %>% 
  tab_model()
...
library(effsize)
cohen.d(efsa_00$idependence.industry.efsa~efsa_00$treatment)
...
efsa_00 %>% 
  drop_na(idependence.polit.efsa) %>% 
ggplot(aes(idependence.polit.efsa,fill=factor(ban.pesticide.lab)))+ 
  geom_bar(width = 0.75,alpha=0.8)+
...
efsa_00 %>% 
  drop_na(idependence.industry.efsa) %>% 
ggplot(aes(idependence.industry.efsa,fill=factor(ban.pesticide.lab)))+ 
  geom_bar(width = 0.75,alpha=0.8)+
  scale_x_continuous(name="Independence from industry",limits = c(-6,6),breaks = -5:5)+
  scale_fill_manual(values = c("dodgerblue2","orange2"))+
  guides(fill=FALSE)+
...
efsa_00 %>% 
  drop_na(ban.pesticide) %>% 
ggplot(aes(x=ban.pesticide.lab,y=credibility.pest.index))+
  geom_boxplot()+
...
t1.2 = t1 %>%
  ...
  focus(credibility.pest.index) %>%
  mutate(rowname = reorder(rowname, credibility.pest.index)) %>%
  mutate(ban.pesticide.lab = "Low risk")
...
ggplot(aes(rowname, credibility.pest.index,fill=ban.pesticide.lab)) +
  geom_col(alpha=0.8,width = 0.75,show.legend = F) +
  scale_y_continuous(name="correlation with credibility Index",
                     limits = c(-0.7,0.7))+
  coord_flip()+
  facet_wrap(~ban.pesticide.lab)+
  theme_tufte()
...
<br>
```{r}
model.pest.credibility.lowrisk <- lm(credibility.pest.index~ 
         credibility.efsa.pre+
         supports.pesticides+
         greediness.food.industry+
           greediness.industry,
...
model.pest.credibility.highrisk <- lm(credibility.pest.index~ 
         credibility.efsa.pre+
         supports.pesticides+
         greediness.food.industry+
           greediness.industry,
...
dwplot(list(LowRisk=model.pest.credibility.lowrisk,HighRisk=model.pest.credibility.highrisk),
       vline = geom_vline(xintercept = 0, colour = "grey60", linetype = 2))+
...
```{r}
sjstats::grpmean(filter(efsa_00,ban.pesticide==0),
                 credibility.pest.index,
                 treatment,
                 out="v")
```
...
```{r}
sjstats::grpmean(filter(efsa_00,ban.pesticide==1),
                 credibility.pest.index,
                 treatment,
                 out="v")
```
