...
After reading the description of EFSA's risk assessment, respondents are asked 6 questions about the credibility of decision. Thereafter, they are asked about the independence of EFSA from politicians, as well as from the industry. We also asked them several question about the credibility of EFSA in general and its reputation (Overman, Busuioc and Wood, forthcoming). We then asked them a series of questions about their familiarity with the four agencies mentioned at the beginning of the survey, and their relations of them. We also asked them about the perceived independence of the three other agencies. 

Towards the end of the survey, we asked them a set of demographic questions and questions about their professional background. Finally, we asked a comprehension-check question about the case.        
...
Our second explanatory variable is stakeholders' prior beliefs/positions regarding the policy issue. To account for stakeholders' pre-determined positions regarding the risk of pesticides, we asked directly asked participants in the whether the use of pesticides "mainly brings benefits and opportunities, or rather poses a threat". Participants answered on 11 points scale ranging between -5 (mainly poses a threat) to +5 (mainly brings benefits). This variable, which represents a controversial issue, has a relatively high variation (*Mean* = `r efsa_00%>%summarise(mean(supports.pesticides))%>%max()%>%round(2)`, *SD* = `r efsa_00%>%summarise(sd(supports.pesticides))%>%max()%>%round(2)`, *Median* = `r efsa_00%>%summarise(median(supports.pesticides))%>%max()%>%round(2)`). As anticipated, these views are associated with the distinction between NGOs and industry organizations. `r figure.x=figure.x+1`**Figure.x `r figure.x`** shows the distribution of the variable across the three groups. As shown by the plot, stakeholders from NGOs hold relatively negative beliefs towards the use of pesticides (only `((efsa_00 %>% filter(organization.type.short=="2-ngo",supports.pesticides>0) %>% nrow())/(efsa_00 %>% filter(organization.type.short=="2-ngo") %>%  drop_na(supports.pesticides) %>% nrow())*100) %>% round(0)`% of them were above zero), whereas stakeholders affiliated with the food industry tend to hold a positive view (`((efsa_00 %>% filter(organization.type.short=="1-industry",supports.pesticides<0) %>% nrow())/(efsa_00 %>% filter(organization.type.short=="1-industry") %>%  drop_na(supports.pesticides) %>% nrow())*100) %>% round(0)`% of them reported below zero). Stakeholders from other organizations were spread along the two sides of the scale. In our analyses below, we use both the support pesticide item and the organization groups as measures for participants' prior positions about regulation of risk of pesticides. Furthermore, our random assignment for the two opinions of EFSA (i.e. to ban or approve the pesticide), provides an additional robust check for the role of these prior beliefs. We expect an interaction between the decision manipulation and prior beliefs. 
...
efsa_00 %>%
  ...
  theme_tufte()+
  labs(title = "Fig. X: Prior beliefs regarding pesticides across organization type",
              subtitle = "",
              caption = "Note: Red dots and bars represent means and 95% CIs")+
  theme(
  plot.caption = element_text(hjust = 0)
...
In addition to the independent variables above, the survey also included control items regarding respondents’ age, gender, educational background, support for EU political institutions, familiarity and intensity of interactions with EFSA and native language. Summary statistics and correlation matrix for all variables are available in the supplementary appendix [## TBA ##].
