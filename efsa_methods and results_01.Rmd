...
After reading the description of EFSA's risk assessment, respondents are asked about the credibility of the opinion, as well as about the independence of EFSA in general. Thereafter, they are asked additional general question about the credibility of EFSA in general and its reputation (Overman, Busuioc and Wood, forthcoming). The subjects are then asked about their familiarity with the four agencies mentioned at the beginning of the survey, and the frwquency of interactions with them. Towards the end of the survey, we asked them a set of demographic questions and questions about their professional background. Finally, we asked a comprehension-check question about the case.  
...
<br>
**Figure `r figure.x`: Prior beliefs regarding pesticides across stakeholder gropus**
...
We begin with exploring the observational relations between stakeholders perceptions about the independence of EFSA from politicians and their perceptions about its credibility. `r figure.x=figure.x+1`**Figure `r figure.x`** graphically shows the bivariate correlations between perceived independence and the three aspects of decision credibility. Idneed, perceived independence is  positively correlated with all three aspects. The correlation is stronger for the first dimension of considerations (*r* = `r cor.test(efsa_00.t1$credibility.pest.considerations,efsa_00.t1$independence.polit.efsa)[["estimate"]][["cor"]] %>% round(2)`, *p*<.01), compared with the aspects of expertise (*r* = `r cor.test(efsa_00.t1$credibility.pest.expertise,efsa_00.t1$independence.polit.efsa)[["estimate"]][["cor"]] %>% round(2)`, *p*<.01) and commitment dimensions (*r* = `r cor.test(efsa_00.t1$credibility.pest.commitment,efsa_00.t1$independence.polit.efsa)[["estimate"]][["cor"]] %>% round(2)`, *p*<.01). 
...
Next, we conduct a multivariate regression analysis, for the three aspects. We examine the effect participants' prior positions on pesticides (in interaction with the decision manipulation), over and above the effect of perceived independence. `r table.x=table.x+1`**Table `r table.x`** shows the regression analyses. For each aspect, we include 4 regression models. First, we regress the variable on perceived independence (Models 1,5,9). Thereafter, we add the decision manipulation (0 = approve pesticide; 1 = ban pesticide) (Models 2,6,10), and the indicators for participants' prior positions - their support for pesticide and the stakeholders groups (the reference category for the groups is "other") (Models 3,7,11). Finally, we add the interaction bewteen the decision manipulation and the prior positions indicators (Models 4,8,12). For all three aspects, the interaction between support for pesticide and ban pesticide is negative and significant. Congruently, the interaction bewteen NGOs and tha ban pesticide decision is positive and significant with regard to considerations and expertise. These interactions indicate that over and above the effect of stakeholders' perceptions about the independence of the agency, participants are significantly more likely to perceive the decision as credible inasmuch as it corresponds to their prior positions. The more a stakeholder perceives pesticides as a safe method, the more she is likely to perceive EFSA's decision to approve a pesticide as more credible then its decision to ban it. Similarly, stakeholders from NGOs tend to perceive a decision to ban a pesticide as more credible then a decision to approve it.   
...
`r figure.x=figure.x+1`**Figure `r figure.x`** graphically shows the interaction between decision manipulation and support for pesticides. Interflex. 
...
`r figure.x=figure.x+1`**Figure `r figure.x`** graphically shows the interaction between decision manipulation and stakeholders' groups. 

<br>
**Figure `r figure.x`: Interaction between decision and stakeholders' groups**
...
stargazer(tmod1.1stage,
          ...
          notes.append = FALSE,
          title = "Table X: Decision credibility. 2SLS and OLS")
