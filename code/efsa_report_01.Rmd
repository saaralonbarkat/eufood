...
efsa_00 %>%
  drop_na(independence.polit.efsa) %>% 
  select(independence.polit.efsa,independence.industry.efsa,
         treatment.lab) %>%
...
## Perceived independence vs. priors

```{r}
tmod1 <- lm(credibility.pest.commitment~ 
            independence.polit.efsa+
              supports.pesticides,
            data=filter(efsa_00.t1,ban.pesticide==0)) %>% 
  std_beta() %>% 
...
tmod2 <- lm(credibility.pest.commitment~ 
            independence.polit.efsa+
              supports.pesticides,
            data=filter(efsa_00.t1,ban.pesticide==1))%>% 
  std_beta() %>% 
    mutate(estimate = std.estimate,
           model = "Ban")
...
models <- rbind(tmod1,
                tmod2) %>% 
  filter(term=="supports.pesticides") %>% 
  mutate(term = term %>% Recode("'supports.pesticides'=''"))

p1 = 
  dwplot(models,dodge_size=0.2,
       vline = geom_vline(xintercept = 0, colour = "grey60", linetype = 2))+
  scale_color_manual(values = c("dodgerblue4","orange2"))+
  theme_tufte()+
  scale_x_continuous(limits = c(-0.3,1),breaks = seq(-0.25,0.75,0.25))+
  ggtitle("Stability")+
...
p3 = 
  dwplot(models,
       ...
       vline = geom_vline(xintercept = 0, colour = "grey60", linetype = 2))+
    scale_x_continuous(limits = c(-0.3,1),breaks = seq(-0.25,0.75,0.25))+
  scale_color_manual(name = "output", values = c("dodgerblue4","orange2"))+
  theme_tufte()+
